  ...
  <div class="app-container">
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :model="temp" :rules="setRules" label-position="left" label-width="90px" style="width: 400px; margin-left:50px;">
        ...
        <el-form-item v-if="dialogStatus==='create'||dialogStatus==='update'" label="活动">
          <el-select v-model="temp.game" multiple placeholder="选择管理的活动">
            <el-option
  ...
  methods: {
    getActivities() {
      getActivity().then(res => {
        this.activityOptions = res.data.results.map(item => {
          return {
            label: item.title,
            value: `${item.id}`
          }
      ...
      getUser(this.listQuery).then(response => {
        ...
        if (tempRes.length > 0) {
          tempRes.forEach(item => {
            ...
            item['game'] = item['game'] === '' ? [] : item['game'].split('|')
          })
        }
        this.list = tempRes
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          temData['game'] = temData['game'] === [] ? '' : temData['game'].join('|')
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          tempData['game'] = tempData['game'] === [] ? '' : tempData['game'].join('|')
